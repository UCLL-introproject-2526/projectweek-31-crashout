...
try:
    ...
    pygame.draw.rect(ROAD_IMG, (50, 50, 50),
                     (WINDOW_WIDTH//6, 0, int(WINDOW_WIDTH*0.66), WINDOW_HEIGHT))
...
# --- 7. PARTICLES: SMOKE, SNOW, TIRE MARKS ---
...
class PlayerCar(pygame.sprite.Sprite):
    def __init__(self, image_path, tire_group):
        ...
        self.accel = 1.2
        self.friction = 0.85
        self.max_side_speed = 20.0

        self.has_shield = False
        self.tire_group = tire_group
        self.trail_timer = 0

    def _spawn_tire_marks(self):
        offset = self.rect.width * 0.25
        back_y = self.rect.bottom - 5
        left_pos  = (self.rect.centerx - offset, back_y)
        right_pos = (self.rect.centerx + offset, back_y)
        self.tire_group.add(TireMark(left_pos))
        self.tire_group.add(TireMark(right_pos))
...
class Explosion(pygame.sprite.Sprite):
    def __init__(self, center, style="fire"):
        ...
        self.particles = []
        if style == "blue":
            colors = [(0, 255, 255), (0, 150, 255), (0, 50, 255),
...
def draw_background(surface, scroll_y):
    if USE_CUSTOM_ROAD:
        ...
        pygame.draw.rect(surface, (50, 50, 50),
                         (ROAD_X, 0, ROAD_WIDTH, WINDOW_HEIGHT))
        pygame.draw.line(surface, (255, 255, 255),
                         (ROAD_X, 0), (ROAD_X, WINDOW_HEIGHT), 5)
        pygame.draw.line(surface, (255, 255, 255),
                         (ROAD_X + ROAD_WIDTH, 0),
                         (ROAD_X + ROAD_WIDTH, WINDOW_HEIGHT), 5)

# --- 10. USERNAME & LEADERBOARD ---
...
def leaderboard():
    while True:
        ...
        if not leader_list:
            ...
            SCREEN.blit(no_score,
                        (WINDOW_WIDTH//2 - no_score.get_width()//2, start_y))
        else:
            for i, entry in enumerate(leader_list[:5]):
                ...
                SCREEN.blit(txt,
                            (WINDOW_WIDTH//2 - txt.get_width()//2, start_y))
                start_y += 60
...
def shop():
    ...
    while True:
        ...
        MONEY_TEXT = get_font(30).render(f"Coins: {GAME_DATA['coins']}",
                                         True, (255, 215, 0))
        ...
        for car in CAR_SHOP:
            ...
            try:
                ...
                car_preview = pygame.transform.scale(car_preview,
                                                     (150, int(150*asp)))
            except:
                car_preview = pygame.Surface((150, 80))
                car_preview.fill((100, 100, 100))

            preview_rect = car_preview.get_rect(
                center=(int(WINDOW_WIDTH * 0.25), y_pos))
            ...
            NAME_RECT = NAME_TXT.get_rect(
                center=(int(WINDOW_WIDTH * 0.5), y_pos))
            SCREEN.blit(NAME_TXT, NAME_RECT)

            BTN = Button(image=None, pos=(int(WINDOW_WIDTH * 0.75), y_pos),
                         text_input=status_text, font=get_font(30),
                         base_color=color, hovering_color="Gray")
    ...
    while True:
        ...
        if not game_over:
            if countdown:
                ...
                hits = pygame.sprite.spritecollide(
                    player, coins, True, pygame.sprite.collide_mask)
                for hit in hits:
                    if COIN_SFX and GAME_DATA["sound"]:
                        COIN_SFX.play()
                    coins_collected += 1
                    score += 100
                ...
                shield_hits = pygame.sprite.spritecollide(
                    player, powerups, True, pygame.sprite.collide_mask)
                for hit in shield_hits:
                    if POWERUP_SFX and GAME_DATA["sound"]:
                        POWERUP_SFX.play()
        ...
        if not crashed and not game_over and player.has_shield:
            pygame.draw.circle(SCREEN, (0, 200, 255),
                               player.rect.center, 75, 4)

        score_text = get_font(30).render(
            f"SCORE: {score}", True, (255, 255, 255))
        coin_text = get_font(30).render(
            f"COINS: {coins_collected}", True, (255, 215, 0))
        ...
        if unlock_message_timer > 0:
            ...
            if (unlock_message_timer // 10) % 2 == 0:
                msg = get_font(40).render(
                    "SECRET UNLOCKED: X-MAS SLED!", True, (0, 255, 0))
        ...
        if game_over:
            if not score_saved:
                ...
                GAME_DATA["leaderboard"].append(
                    {"name": GAME_DATA["username"], "score": score})
                GAME_DATA["leaderboard"].sort(
            ...
            final_coin = get_font(40).render(
                f"Collected: {coins_collected}", True, (255, 215, 0))
            restart_text = get_font(30).render(
                "Press 'R' to Restart", True, (255, 255, 255))

            SCREEN.blit(go_text,
                        (WINDOW_WIDTH//2 - go_text.get_width()//2,
                         WINDOW_HEIGHT//2 - 80))
            SCREEN.blit(final_coin,
                        (WINDOW_WIDTH//2 - final_coin.get_width()//2,
                         WINDOW_HEIGHT//2))
...
def main_menu():
    ...
    while True:
        ...
        user_txt = get_font(20).render(
            f"Player: {GAME_DATA['username']}", True, "White")
        ...
        CARS_BUTTON = Button(image=None, pos=(WINDOW_WIDTH // 2, 280),
                             text_input="CARS", font=get_font(50),
                             base_color="#d7fcd4", hovering_color="White")

        LEADER_BUTTON = Button(image=None, pos=(WINDOW_WIDTH // 2, 360),
                               text_input="LEADERBOARD", font=get_font(40),
                               base_color="#d7fcd4", hovering_color="White")
        ...
        SOUND_BUTTON = Button(image=None, pos=(WINDOW_WIDTH // 2, 440),
                              text_input=sound_text, font=get_font(40),
                              base_color=sound_color, hovering_color="Green")
        ...
        RESET_BUTTON = Button(image=None, pos=(WINDOW_WIDTH // 2, 580),
                              text_input="RESET PROGRESS", font=get_font(30),
                              base_color="Red", hovering_color="#ff5555")

        SCREEN.blit(MENU_TEXT, MENU_RECT)

        for button in [PLAY_BUTTON, CARS_BUTTON, LEADER_BUTTON,
                       SOUND_BUTTON, QUIT_BUTTON, RESET_BUTTON]:
